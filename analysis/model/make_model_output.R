...
files_S <- list.files(path = model_dir, pattern = "stata_model_output-")
...
for (j in files_S) {
  ...
  stata_model_output <- readr::read_tsv(file = paste0(model_dir,j), skip = 2,
                                        col_names = c("term",
                                                      "b_min","se_min","t_min",
  ...
  median_fup <- readr::read_csv(file = paste0(model_dir,gsub(".txt",".csv",gsub("stata_model_output-","stata_fup-",j))))
