...
lifetable <- function(model_output, aer_input, day0) {
    ...
    model_output <- model_output[
        model_output$analysis == aer_input$analysis &
    ...
    lifetable$cumulative_survival_unexp <- cumprod(
        1 - lifetable$incidence_unexp
    )
    ...
    for (i in 1:nrow(model_output)) {
        tmp <- model_output[i, ]
        lifetable$hr <- ifelse(
            lifetable$days >= tmp$time_period_start &
                lifetable$days < tmp$time_period_end,
            tmp$hr,
            lifetable$hr
        )
    }
    ...
    lifetable$cumulative_survival_exp <- cumprod(
        1 - (lifetable$hr * lifetable$incidence_unexp)
    )

    # Step 5. Calculate the daily excess risk ------------------------------------
    print('Step 5. Calculate the daily excess risk')

    lifetable$cumulative_difference_absolute_excess_risk <- lifetable$cumulative_survival_unexp -
        lifetable$cumulative_survival_exp
